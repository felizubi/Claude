...
if ( class_exists( 'SucomUpdate' ) ) {
	...
	class SucomUpdate {
		...
		private static $api_version = '5.6';
		...
		public function __construct( &$plugin, &$addon, $ext_text_domain = '' ) {
			...
			if ( isset( $this->p->id ) ) {
				...
				if ( isset( $this->p->cf[ 'plugin' ][ $this->p_id ][ 'slug' ] ) ) {	// Just in case.
					...
					$this->p_ucfg_csum   = $this->p_id . '_um_chksum';
		...
		public function clear_upd_config() {
			...
			if ( ! empty( $this->p_ucfg_name ) ) delete_option( $this->p_ucfg_name );

			if ( ! empty( $this->p_ucfg_csum ) ) delete_option( $this->p_ucfg_csum );
		}
		 ...
		 */
		public function refresh_upd_config( $quiet = false ) {
		...
		private function set_upd_config( $quiet = false, $read_cache = true ) {
			...
			if ( $read_cache ) {
				...
				if ( ! empty( $this->p_ucfg_name ) ) {	// Just in case.
					...
					self::$upd_config = get_option( $this->p_ucfg_name );	// Option is autoloaded.

					if ( is_array( self::$upd_config ) ) {	// Just in case.

						$checksum = get_option( $this->p_ucfg_csum );	// Option is autoloaded.

						if ( md5( serialize( self::$upd_config ) === $checksum ) ) {
							...
							$this->p->debug->log( 'update manager config failed checksum' );
						}
					}
				}
			}
			...
			foreach ( $this->p->cf[ 'plugin' ] as $ext => $info ) {
				...
				if ( 'none' !== $ext_auth_type && empty( $ext_auth_id ) ) {
					...
					if ( $this->p->debug->enabled ) {
						...
						$this->p->debug->log( $ext . ' plugin: skipped - auth type without auth id' );
					}
				...
				} else $filter_name = $this->get_ext_filter_name( $ext );
			...
			if ( ! empty( $this->p_ucfg_name ) ) {	// Just in case.
				...
				update_option( $this->p_ucfg_csum, md5( serialize( self::$upd_config ) ), $autoload = true );
			}
		...
		/*
		 * Called by WpssoSubmenuLicenses->show_metabox_licenses() to show the exp_date, qty_used, qty_reg, and qty_total
		 * property values.
		 *
